...
{
    W('<div class="tomato-refresh form-inline input-append">');
    if (spin) W('<img src="spin.gif" id="refresh-spinner">');
    genStdTimeList('refresh-time', 'Auto Refresh', min);
    W('<button value="Refresh" onclick="' + (exec ? exec : 'refreshClick()') + '; return false;" id="refresh-button" class="btn">Refresh</button></div>');
}
...
function _tabCreate(tabs)
{
    ...
    for (var i = 0; i < arguments.length; ++i)
        buf.push('<a class="btn" style="height: 20px; margin: -1px -1px 0 0; border-radius: 0;" href="javascript:tabSelect(\'' + arguments[i][0] + '\')" id="' + arguments[i][0] + '">' + arguments[i][1] + '</a>');
    buf.push('</div>');
    return buf.join('');
}
...
function navi()
{
    var menu = [
        ...
        ['Bandwidth', 			'bwm', 0, [            
                ...
                ['Weekly',			'weekly.asp'],
                ['Monthly',			'monthly.asp'],
                ['IP Traffic',       'devide'], // Devide and Show Header to the following 
                ['Real-Time',        'ipt-realtime.asp'],
                ['Last 24 Hours',    'ipt-24.asp'],
                ['View Graphs',            'ipt-graphs.asp'],
                ['Transfer Rates',    'ipt-details.asp'],
                ['Daily',            'ipt-daily.asp'],
                ['Monthly',            'ipt-monthly.asp'] ] ],
        ['Tools', 				'tools', 0, [
                ['Ping',			'ping.asp'],
                ['Trace',			'trace.asp'],
                ['System',			'shell.asp'],
...
/* SNMP-END */
                ['Upgrade',			'upgrade.asp'],
                ['Router Control',  'devide'],
                ['<i class="icon-refresh"></i> Reboot...',            'javascript:reboot()'],
                ['<i class="icon-off"></i> Shutdown...',            'javascript:shutdown()'],
                ['<i class="icon-share"></i> Logout',                'javascript:logout()']
            ]
        ],
...
function fixNavi() {
    ...
    var isNAV = $('.navi').length;

    if (wWidth <= 680 && isNAV >= 1) {
         
        $('.navi > ul > li > a').click(function() {

                var cClass = $(this).parent('li').is('.clicked');

                $('.navi ul li').removeClass('clicked');
                if (!cClass) { $(this).parent('li').addClass('clicked'); }
                return false;

        });

        $('.#main .span12').click(function() {
                $('.navi > ul > li').removeClass('clicked');
        });

    } else {
        $('.navi > ul > li').removeClass('clicked');
        $('.navi > ul > li').unbind('click');
    }
}
...
function backToTop() {
    ...
    if (toTop === null) {
        $('body').append('<a title="Scroll to TOP" href="#" id="toTop"><i class="icon-chevron-up icon-white"></i></a>');
    }
...
function AdvancedTomato() {
    ...
    if (typeof nvram.at_update !== "undefined" && nvram.at_update != '') {
        ...
        if (n < lastUpdate || n == null) {
            $("#main .span12").prepend('<div class="alert alert"><button type="button" class="close" data-update="' + lastUpdate + '">×</button>'+
                'There is new AdvancedTomato (' + nvram.at_update + ') update available. <br /><a target="_blank" href="http://at.prahec.com/">Click here to read more</a>.</div>');
        }
    }
    ...
    if (typeof nvram.tomatoanon_answer !== "undefined") {          
        if (nvram.tomatoanon_answer != '1') {
            $('#main .span12').prepend('<div class="alert anon"><h4>Attention</h4>You did not configure <b>TomatoAnon project</b> setting.<br />'+
                'Please go to <a href="admin-tomatoanon.asp">TomatoAnon configuration page</a> and make a choice.');
